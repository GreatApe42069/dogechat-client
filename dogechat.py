...
# UUIDs
DOGECHAT_SERVICE_UUID = "f47b5e2d-4a9e-4c5a-9b3f-8e1d2c3a4b5c"
DOGECHAT_CHARACTERISTIC_UUID = "a1b2c3d4-e5f6-4a5b-8c9d-0e1f2a3b4c5d"
...
class DogechatClient:
    ...
    async def find_device(self) -> Optional[BLEDevice]:
        """Scan for Dogechat service"""
        debug_println("[1] Scanning for dogechat service...")
        
        devices = await BleakScanner.discover(
            timeout=5.0,
            service_uuids=[DOGECHAT_SERVICE_UUID]
        )
    ...
    def handle_disconnect(self, client: BleakClient):
        ...
        print(f"\r\033[K\033[91m✗ Disconnected from Dogechat network\033[0m")
    ...
    async def connect(self):
        ...
        while not device and self.running:
            ...
            if not device:
                ...
                if scan_attempts == max_initial_attempts:
                    print("\033[93m» No other Dogechat devices found yet.\033[0m")
        ...
        print("\033[90m» Found dogechat service! Connecting...\033[0m")
        ...
        try:
            ...
            for service in services:
                for char in service.characteristics:
                    if char.uuid.lower() == DOGECHAT_CHARACTERISTIC_UUID.lower():
            ...
            print("\033[90m  • The other device is running Dogechat\033[0m")
    ...
    async def handshake(self):
        ...
        if self.client and self.characteristic:
            ...
            try:
                ...
                identity_packet = create_dogechat_packet_with_signature(
                ...
                handshake_packet = create_dogechat_packet(
            ...
            # Send announce
            announce_packet = create_dogechat_packet(
    ...
    async def send_packet_with_fragmentation(self, packet: bytes):
        ...
        for index, chunk in enumerate(chunks):
            ...
            fragment_packet = create_dogechat_packet(
                self.my_peer_id,
                fragment_type,
    ...
    async def notification_handler(self, sender: BleakGATTCharacteristic, data: bytes):
        ...
        try:
            packet = parse_dogechat_packet(data)
    ...
    async def handle_packet(self, packet: DogechatPacket, raw_data: bytes):
    ...
    async def handle_announce(self, packet: DogechatPacket):
        ...
        if is_new_peer:
            ...
            if self.my_peer_id < packet.sender_id_str:
                ...
                try:
                    ...
                    handshake_packet = create_dogechat_packet_with_recipient(
                    ...
                    key_exchange_packet = create_dogechat_packet(
                ...
                try:
                    ...
                    identity_packet = create_dogechat_packet_with_recipient(
    ...
    async def handle_message(self, packet: DogechatPacket, raw_data: bytes):
        """Handle chat message"""
        ...
        try:
            if is_private_message and decrypted_payload:
                ...
                message = parse_dogechat_message_payload(unpadded)
            else:
                message = parse_dogechat_message_payload(packet.payload)
    ...
    async def display_message(self, message: DogechatMessage, packet: DogechatPacket, is_private: bool):
    ...
    async def handle_fragment(self, packet: DogechatPacket, raw_data: bytes):
        ...
        if len(packet.payload) >= 13:
            ...
            if result:
                ...
                reassembled_packet = parse_dogechat_packet(complete_data)
    ...
    async def handle_key_exchange(self, packet: DogechatPacket):
        """Handle key exchange"""
        try:
            ...
            if response:
                response_packet = create_dogechat_packet(
    ...
    async def handle_noise_handshake_init(self, packet: DogechatPacket):
        ...
        try:
            ...
            if response:
                ...
                response_packet = create_dogechat_packet_with_recipient(
    ...
    async def handle_noise_handshake_resp(self, packet: DogechatPacket):
        ...
        try:
            ...
            if response:
                ...
                final_packet = create_dogechat_packet_with_recipient(
    ...
    async def handle_noise_encrypted(self, packet: DogechatPacket, raw_data: bytes):
        ...
        try:
            ...
            # The decrypted payload should be a complete DogechatPacket (matching Swift implementation)
            # Swift creates: DogechatPacket(type: MessageType.message, ...) and encrypts the whole packet
            
            try:
                # Check if the decrypted data starts with version 1 (DogechatPacket)
                if len(decrypted_payload) > 0 and decrypted_payload[0] == 1:
                    # Parse the decrypted data as a complete DogechatPacket
                    inner_packet = parse_dogechat_packet(decrypted_payload)
                    if inner_packet:
                        ...
                        if inner_packet.msg_type == MessageType.MESSAGE:
                            ...
                            try:
                                message = parse_dogechat_message_payload(inner_packet.payload)
                        ...
                        debug_println(f"[NOISE] Failed to parse decrypted data as DogechatPacket")
                else:
                    # Handle non-DogechatPacket data (likely JSON acknowledgments or receipts)
    ...
    async def handle_leave(self, packet: DogechatPacket):
    ...
    async def handle_channel_announce(self, packet: DogechatPacket):
    ...
    async def handle_delivery_ack(self, packet: DogechatPacket, raw_data: bytes):
    ...
    async def handle_noise_identity_announce(self, packet: DogechatPacket):
    ...
    async def send_delivery_ack(self, message_id: str, sender_id: str, is_private: bool):
        ...
        # Send ACK packet
        ack_packet = create_dogechat_packet_with_recipient(
            self.my_peer_id,
            sender_id,
    ...
    async def handle_user_input(self, line: str):
        ...
        if line == "/exit":
            ...
            if self.client and self.client.is_connected:
                leave_packet = create_dogechat_packet(
        ...
        if line in ["/online", "/w"]:
            if not self.client or not self.client.is_connected:
                ...
                print("\033[90mWaiting for other Dogechat devices...\033[0m")
            else:
    ...
    async def handle_dm_command(self, line: str):
        """Handle /dm command"""
        if not self.client or not self.client.is_connected:
            print("\033[93m⚠ Not connected to the Dogechat network yet.\033[0m")
    ...
    async def handle_pass_command(self, line: str):
        ...
        if old_key:
            ...
            try:
                ...
                notify_packet = create_dogechat_packet(self.my_peer_id, MessageType.MESSAGE, notify_payload)
        ...
        init_packet = create_dogechat_packet(self.my_peer_id, MessageType.MESSAGE, init_payload)
    ...
    async def send_public_message(self, content: str):
        ...
        if current_channel and current_channel in self.channel_keys:
            ...
            payload, message_id = create_dogechat_message_payload_full(
        ...
        message_packet = create_dogechat_packet(
    ...
    async def send_private_message(self, content: str, target_peer_id: str, target_nickname: str, message_id: Optional[str] = None):
        ...
        # Create INNER packet (DogechatPacket with MESSAGE type) that will be encrypted
        # This matches Swift implementation: DogechatPacket(type: MessageType.message, ...)
        inner_packet = create_dogechat_packet_with_recipient(
            self.my_peer_id,
            target_peer_id,
            MessageType.MESSAGE,
            payload,
        ...
        try:
            ...
            packet = create_dogechat_packet_with_recipient(
                self.my_peer_id,
                target_peer_id,
    ...
    async def background_scanner(self):
        ...
        while self.running:
            ...
            if not self.client or not self.client.is_connected:
                ...
                device = await self.find_device()
                if device:
                    print(f"\r\033[K\033[92m» Found a Dogechat device! Connecting...\033[0m")
                    try:
                        self.client = BleakClient(device.address, disconnected_callback=self.handle_disconnect)
                        await self.client.connect()
                        ...
                        if self.characteristic:
                            # Subscribe to notifications
                            await self.client.start_notify(self.characteristic, self.notification_handler)
                            print(f"\r\033[K\033[92m✓ Connected to Dogechat network!\033[0m")
...
def parse_dogechat_packet(data: bytes) -> DogechatPacket:
    """Parse a Dogechat packet from raw bytes"""
    HEADER_SIZE = 13
    SENDER_ID_SIZE = 8
    RECIPIENT_ID_SIZE = 8
...
def parse_dogechat_message_payload(data: bytes) -> DogechatMessage:
    ...
    return DogechatMessage(id_str, content, channel, is_encrypted, encrypted_content)

def create_dogechat_packet(sender_id: str, msg_type: MessageType, payload: bytes) -> bytes:
    """Create a Dogechat packet"""
    return create_dogechat_packet_with_recipient(sender_id, None, msg_type, payload, None)

def create_dogechat_packet_with_signature(sender_id: str, msg_type: MessageType, 
                                        payload: bytes, signature: Optional[bytes]) -> bytes:
    """Create a Dogechat packet with signature"""
    return create_dogechat_packet_with_recipient(sender_id, None, msg_type, payload, signature)

def create_dogechat_packet_with_recipient_and_signature(sender_id: str, recipient_id: str,
                                                      msg_type: MessageType, payload: bytes,
                                                      signature: Optional[bytes]) -> bytes:
    """Create a Dogechat packet with recipient and signature"""
    return create_dogechat_packet_with_recipient(sender_id, recipient_id, msg_type, payload, signature)

def create_dogechat_packet_with_recipient(sender_id: str, recipient_id: Optional[str],
                                       msg_type: MessageType, payload: bytes,
                                       signature: Optional[bytes]) -> bytes:
    """Create a Dogechat packet with all options"""
...
def create_dogechat_message_payload_full(sender: str, content: str, channel: Optional[str],
...
def create_encrypted_channel_message_payload(sender: str, content: str, channel: str, key: bytes, encryption_service, sender_peer_id: str) -> Tuple[bytes, str]:
    ...
    return create_dogechat_message_payload_full(sender, content, channel, False, sender_peer_id, True, encrypted_content)
